      ...
      <div className="panel">
        <div className="mb-5 flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold text-white light:text-slate-950">
              Transactions
            </h2>
          </div>
        ...
        <div className="overflow-hidden rounded-md border border-white/10">
          ...
          {!loading &&
            items.map((item) => (
              <div
                key={item._id}
                className="grid gap-3 border-b border-white/10 px-4 py-4 last:border-0 md:grid-cols-[1.2fr_1fr_120px_130px_96px] md:items-center"
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-white light:text-slate-950">
                    {item.title}
                  </p>

                  <p className="truncate text-xs text-slate-500">
                    {item.merchant || "No merchant"} / {item.source}
                  </p>
                </div>
          ...
          {!loading && items.length === 0 && (
            <div className="p-10 text-center">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Adjust your filters or add a transaction to begin.
              </p>
            </div>
          )}
        </div>
